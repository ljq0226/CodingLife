...
```ad-note
Controllers 负责处理传入的 requests 和向客户端返回 responses。
Http 请求由不同的 controller 共同进行处理，如 userController 处理对应的 user 相关的请求(createUser、updateUser...)
```
...
```ad-note
+ 路由path由两部分组成:
		1. @Controller() 中的 prefix path
		2. @Get() 等 Http请求方法中的specify path
+ 对应路由返回对应 function 的返回值， function 的命名是无关紧要的 
+ 返回值若为 object/array 则会被序列化为JSON ,若为其它值(string,number,boolean) 则直接返回其value
...
![](https://obs-pic-1309372570.cos.ap-chongqing.myqcloud.com/20221003222413.png)
